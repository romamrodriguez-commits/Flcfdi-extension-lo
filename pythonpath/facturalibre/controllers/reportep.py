...
class EventosReporteP(object):
    ...
    def cmdGenerar(self):
        ...
        if not self.dm.chkDesglozado.State:
            campos = ('compras.fecha, compras.serie || compras.folio, compras.estatus, receptores.nombre, compras.subtotal, compras.descuento, compras.totalImpuestosTrasladados, compras.total',)
            tablas = ('compras', )
            ...
            Periodo = 'Reporte de compras por proveedor del %s al %s' % (FechaIni, FechaFin)
        else:
            campos = ('compras.fecha, compras.serie || compras.folio, compras.estatus, receptores.nombre, compradetalle.descripcion, compradetalle.unidad, compradetalle.cantidad, compradetalle.valorUnitario, compradetalle.importe',)
            tablas = ('compradetalle',)
            otro = 'INNER JOIN compras ON compradetalle.id_compra=compras.id INNER JOIN receptores ON receptores.id = compras.id_proveedor'
            titulos = ('Fecha', 'Factura', 'Estado', 'Cliente', 'Descripción', 'Unidad', 'Cantidad', 'Precio unitario', 'Importe')
            Periodo = 'Reporte de compras por producto del %s al %s' % (FechaIni, FechaFin)
        ...
        for row in data:
            fecha_str = row[0]
            fechita = self.util.format_date(fecha_str[0:10], '%d/%m/%Y')
            fecha = self.util.date_to_calc(fechita, True)
