...
class Dlg(object):
    ...
    def __config(self):
        ...
        self.dialog.Title = 'Reporte de productos comprados'
        self.dm.optCliente.Label = 'Productos'
        self.dm.lblCliente.Label = 'Productos'
        self.dm.chkDesglozado.Label = 'Desglozado por proveedor'
        listbox = self.dialog.getControl('lstClientes')
        data=self.db.select(('compradetalle',),('descripcion',),order='descripcion',groupby='descripcion')
        data_ok = [('Todos',)] + data
        if data:
